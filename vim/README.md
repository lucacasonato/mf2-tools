...
After that, `*.mf2` files will use `filetype=mf2` and load syntax
...
## Use `mf2lsp` with `coc.nvim`

`mf2lsp` provides LSP features (diagnostics, completion, rename, etc.). Keep
the Vim syntax files above for classic syntax highlighting.
...
Restart coc (`:CocRestart`) and open a `.mf2` file.
